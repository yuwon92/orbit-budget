...
- **위시 행성에는 장착한 행성 색·무늬만 입힌다**(`wishSkinOf`, `lib/preview.ts`). 허브·퀘스트·도감·완주 연출이 App에서 같은 값을 받는다. 링·배경·동료·효과는 관측소 미리보기·꾸미기에만 — 위시 행성끼리는 씨앗마다 다른 무늬 자리와 진행 단계로 구분한다. 로고·빈 궤도 안내의 행성은 위시가 아니라 입히지 않는다
...
- **움직이는 것은 효과 층뿐이다**(`PixelPreset.anim` — `twinkle`·`rise`·`float`·`pulse`·`drift`). 투명도만 바꾸면 밋밋해서 크기·이동·밝기를 함께 쓴다. **회전은 금지** — `shape-rendering: crispEdges`가 안티에일리어싱을 꺼서 비스듬한 변이 톱니로 깨진다. 배경·동료는 정지다. 여럿이 움직이면 어느 것이 보상으로 얻은 효과인지 구분이 안 된다. 별 하나가 여러 칸이라 `CosmeticPixel.g`로 묶어야 가로줄과 세로줄이 같이 깜빡인다(`drift`는 번호 순서가 곧 빛이 흐르는 방향). 시작 지연은 `PixelPlanet`의 `ANIM_STEP_MS`, 주기와 계단은 `index.css`의 `@keyframes fx-*`. `prefers-reduced-motion`에서 멈춘다
...
- **코스믹 박스 구매는 주 3회**(`BOX_WEEKLY_LIMIT`, 월요일 0시 초기화). 미보유 우선이라 상자 한 장이 상점 아이템 하나로 바뀐다 — 60짜리 21장(1,260)이면 상점 재고 전부(직접 사면 5,120)를 모은다. 가격은 §7 그대로 두고 속도만 묶었다. 한도는 상점에서 산 상자(`shop:…`)만 세고 레벨 보상 상자는 세지 않는다. 판정은 `buyBox`가 트랜잭션 안에서 다시 한다 — 화면을 그린 뒤 다른 탭에서 샀을 수 있다
...
**기간 없는 위시는 보상 없는 저금통이다**(`source: 'piggy'`). 퀘스트 로그 WISH LIST 줄에서 `PiggySheet`로 넣기·꺼내기(꺼내기는 `withdraw` — 이번 달 자유비용으로 회수). 하루 몫 미션·저금한 날·연속 기록에 안 잡히고(`countsAsHabit`), 월 순저금·평생 누적·HUD 저금통에는 들어간다. **`targetDate` 유무로 가르지 않는다** — XP는 파생값이라 나중에 궤도에 올리는 순간 지난 저금통 납입이 소급해 하루 몫 XP를 받는다. 같은 이유로 목표를 채운 기간 없는 위시에는 `더 기다리기`(하루 20 XP)를 내지 않는다. 완주 보너스는 궤도 위시와 같다. 어제 남은 예산 넘기기의 대상에는 들어간다(하루 한 번뿐이라 XP가 불어나지 않는다).
